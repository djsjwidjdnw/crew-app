...
class _JobPostScreenState extends State<JobPostScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadMyJobs() async {
    ...
    try {
      ...
      if (userId == null) { if (mounted) setState(() => _loadingMy = false); return; }
  ...
  Future<void> _deleteJob(String jobId) async {
    ...
    try {
      ...
      await _loadMyJobs();
      if (!mounted) return;
      AppFeedback.showSuccess(context, 'Job deleted');
    } catch (e) {
      if (!mounted) return;
      AppFeedback.showError(context, e);
    }
  ...
  Widget _buildDiscoverTab() {
        ...
        child: ListView(physics: const AlwaysScrollableScrollPhysics(), children: [
          SizedBox(
            ...
            child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [
              ...
              Text('No other jobs posted yet', style: TextStyle(color: Color(0xFFF0F4FF), fontSize: 20, fontWeight: FontWeight.w700)),
              SizedBox(height: 8), Text('Check back later', style: TextStyle(color: Color(0xFF8896b0))),
            ])),
          ),
        ]),
      );
    ...
    return RefreshIndicator(
      onRefresh: _loadDiscoverJobs,
      color: const Color(0xFFFF6B35),
      child: ListView.separated(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(16), itemCount: _allJobs.length, separatorBuilder: (_, __) => const SizedBox(height: 8),
  ...
  Widget _buildMyPostsTab() {
      ...
      return RefreshIndicator(
        ...
        child: ListView(physics: const AlwaysScrollableScrollPhysics(), children: [
          SizedBox(
            ...
            child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [
              Text('📋', style: TextStyle(fontSize: 48)), SizedBox(height: 16),
              Text('No jobs posted yet', style: TextStyle(color: Color(0xFFF0F4FF), fontSize: 20, fontWeight: FontWeight.w700)),
              SizedBox(height: 8), Text('Tap + to post your first job', style: TextStyle(color: Color(0xFF8896b0))),
            ])),
          ),
        ]),
      );
    ...
    return RefreshIndicator(
      onRefresh: _loadMyJobs,
      color: const Color(0xFFFF6B35),
      child: ListView.separated(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(16), itemCount: _myJobs.length, separatorBuilder: (_, __) => const SizedBox(height: 8),
        itemBuilder: (context, index) {
...
class _JobFormPageState extends State<_JobFormPage> {
  ...
  Future<void> _save() async {
    if (_titleController.text.trim().isEmpty) {
      AppFeedback.showError(context, 'Please enter a job title', fallback: 'Please enter a job title');
      return;
    }
    ...
    if (rate == null || rate <= 0) {
      AppFeedback.showError(context, 'Enter a valid hourly rate greater than 0', fallback: 'Enter a valid hourly rate greater than 0');
      return;
    }
    final duration = int.tryParse(durationText);
    if (duration == null || duration <= 0) {
      AppFeedback.showError(context, 'Enter a valid duration in days greater than 0', fallback: 'Enter a valid duration in days greater than 0');
      return;
    }
    setState(() => _saving = true);
    try {
      final userId = Supabase.instance.client.auth.currentUser?.id;
      if (userId == null) { if (mounted) setState(() => _saving = false); return; }
      final data = {
        'journeyman_id': userId, 'title': _titleController.text.trim(), 'description': _descriptionController.text.trim(),
        'location_text': _locationController.text.trim(), 'hourly_rate': rate,
        'duration_days': duration, 'experience_required': _experienceRequired,
      ...
      widget.onSaved();
      if (!mounted) return;
      AppFeedback.showSuccess(context, _isEditing ? 'Job updated!' : 'Job posted!');
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(controller: _titleController, maxLength: 100, style: const TextStyle(color: Color(0xFFF0F4FF)), decoration: const InputDecoration(hintText: 'e.g. Welder Helper Needed')),
          const SizedBox(height: 20),
          _label('DESCRIPTION'), const SizedBox(height: 8),
          TextField(controller: _descriptionController, maxLength: 1000, style: const TextStyle(color: Color(0xFFF0F4FF)), maxLines: 4, decoration: const InputDecoration(hintText: 'Describe the job, requirements, etc.')),
          const SizedBox(height: 20),
          ...
          const SizedBox(height: 20),
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_label('HOURLY RATE (\$)'), const SizedBox(height: 8), TextField(controller: _rateController, maxLength: 6, keyboardType: TextInputType.number, style: const TextStyle(color: Color(0xFFF0F4FF)), decoration: const InputDecoration(hintText: '45'))])),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_label('DURATION (DAYS)'), const SizedBox(height: 8), TextField(controller: _durationController, maxLength: 4, keyboardType: TextInputType.number, style: const TextStyle(color: Color(0xFFF0F4FF)), decoration: const InputDecoration(hintText: '14'))])),
          ]),
          const SizedBox(height: 20),
          ...
          Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(color: const Color(0xFF0A0E1A), borderRadius: BorderRadius.circular(4), border: Border.all(color: const Color(0xFF1e2d45))),
            child: DropdownButtonHideUnderline(child: DropdownButton<String>(
              ...
              items: [DropdownMenuItem(value: 'any', child: Text(CrewConstants.expToLabel('any'))), ...CrewConstants.experienceLevels.map((e) => DropdownMenuItem(value: e['value'], child: Text(e['label']!)))],
